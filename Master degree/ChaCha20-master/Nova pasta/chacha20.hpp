...
struct Chacha20Block {
    ...
    static unsigned long int rotl32(unsigned long int x, int n){
        return (x << n) | (x >> (32 - n));
    }

    static unsigned long int pack4(const unsigned char *a){
        return
            (unsigned long int)(a[0] << 0*8) |
            (unsigned long int)(a[1] << 1*8) |
            (unsigned long int)(a[2] << 2*8) |
            (unsigned long int)(a[3] << 3*8);
    }

    static void unpack4(unsigned long int src, unsigned char *dst){
        dst[0] = (src >> 0*8) & 0xff;
        dst[1] = (src >> 1*8) & 0xff;
        dst[2] = (src >> 2*8) & 0xff;
        dst[3] = (src >> 3*8) & 0xff;
    ...
    Chacha20Block( const unsigned char key[32],const unsigned char nonce[8]){
        const unsigned char *magic_constant = (unsigned char*)"expand 32-byte k";
    ...
    void set_counter(unsigned long long int counter){
        ...
        state[13] = counter >> 32;
        //cout << (unsigned long int) state[12] << " " <<  (unsigned long int) state[13] << endl;
    }

    void next(unsigned long int result[16]){
...
#define CHACHA20_QUARTERROUND(x, a, b, c, d) \
    ...
    x[c] += x[d]; x[b] = rotl32(x[b] ^ x[c], 7);
        ...
        //for (int i = 0; i < 16; i++) cout << result [i] << endl;
        for (int i = 0; i < 16; i++) result[i] += state[i];
...
struct Chacha20 {
    ...
    Chacha20Block block;
    unsigned long int keystream32[16];
    size_t position;
    ...
    void crypt(unsigned char *bytes, size_t n_bytes){
